...
const socket = io("https://fullstack-smart-meter.onrender.com")

const Readings = () => {
  const { state } = useLocation()
  const meterId = state?.meterId // may be undefined
  ...
  useEffect(() => {
    if (!meterId) {
      setMessage("No meter selected yet. Live readings cannot be fetched.")
      return
    }
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1 flex flex-col items-center justify-start px-6 py-10">
        <div className="w-full max-w-2xl bg-white shadow-lg rounded-2xl p-8">
          ...
          {/* Meter Details */}
          {meterId ? <MeterDetails meterId={meterId} /> : <p className="text-gray-500 mb-4">No meter selected.</p>}
          ...
          {selectedReading && (
            <div className="mt-6 bg-gray-50 border border-gray-200 p-5 rounded-xl shadow-sm">
              <h3 className="text-lg font-semibold mb-3 text-gray-800 text-center">Reading Details</h3>
              <div className="text-gray-700 text-sm space-y-1">
                <p><span className="font-semibold">ID:</span> {selectedReading._id}</p>
                <p><span className="font-semibold">Meter:</span> {selectedReading.meter?.name || selectedReading.meter?._id}</p>
                <p><span className="font-semibold">Energy:</span> {selectedReading.energy_consumed} kWh</p>
                <p><span className="font-semibold">Voltage:</span> {selectedReading.voltage} V</p>
                <p><span className="font-semibold">Current:</span> {selectedReading.current} A</p>
                <p><span className="font-semibold">Status:</span> {selectedReading.status}</p>
                <p><span className="font-semibold">Date:</span> {new Date(selectedReading.timestamp).toLocaleString()}</p>
              </div>
            </div>
          )}
